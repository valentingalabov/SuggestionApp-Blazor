...
public class MongoSuggestionData : ISuggestionData
{
   ...
   private readonly IMongoCollection<SuggestionModel> _suggstions;
   ...
   public MongoSuggestionData(IDbConnection db, IUserData userData, IMemoryCache cache)
   {
      ...
      _userData = userData;
      _cache = cache;
      _suggstions = db.SuggestionCollection;
   }
   ...
   public async Task<List<SuggestionModel>> GetAllSuggestions()
   {
      ...
      if (output is null)
      {
         var result = await _suggstions.FindAsync(s => s.Archived == false);
   ...
   {
      var result = await _suggstions.FindAsync(s => s.Id == id);
   ...
   {
      await _suggstions.ReplaceOneAsync(s => s.Id == suggestion.Id, suggestion);
   ...
   public async Task CreateSuggestion(SuggestionModel suggestion)
   {
      ...
      try
      {
         ...
         var usersInTransaction = db.GetCollection<UserModel>(_db.CategoryCollectionName);
         var user = await _userData.GetUser(suggestion.Author.Id);
